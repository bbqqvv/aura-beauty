...
const AddProduct = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!formData.children) return alert("Please select a subcategory");
          ...
          <div className="glass-panel" style={{ marginBottom: '1rem', padding: '1rem' }}>
            ...
            {additionalInfo.map((info, index) => (
              <div key={index} style={{ display: 'flex', gap: '1rem', alignItems: 'center', marginBottom: '1rem' }}>
                <input type="text" value={info.key} onChange={(e) => updateInfo(index, 'key', e.target.value)} placeholder="Thuộc tính (VD: Dung tích)" className="admin-input-premium" style={{ flex: 1, padding: '0.5rem' }} />
                <input type="text" value={info.value} onChange={(e) => updateInfo(index, 'value', e.target.value)} placeholder="Giá trị (VD: 50ml)" className="admin-input-premium" style={{ flex: 2, padding: '0.5rem' }} />
        ...
        <div className="admin-form-sidebar">
          ...
          <div style={{ marginTop: '1.5rem' }}>
            <button type="submit" disabled={isLoading} className="admin-btn admin-btn-primary" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '1rem', fontSize: '1rem', width: '100%', justifyContent: 'center', borderRadius: '8px', boxShadow: '0 4px 12px rgba(9, 137, 255, 0.3)' }}>
              <Save size={20} /> {isLoading ? 'Đang lưu...' : 'Lưu Sản phẩm'}
            </button>
          </div>
